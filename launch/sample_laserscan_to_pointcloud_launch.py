...
def generate_launch_description():
    return LaunchDescription([
        ...
        #     remappings=[('scan_in', [LaunchConfiguration(variable_name='scanner'), '/velodyne_2dscan_high_beams']),
        #                 ('cloud', [LaunchConfiguration(variable_name='scanner'), '/cloud'])],
        #     parameters=[{'target_frame': 'velodyne', 'transform_tolerance': 0.01}]
        # ),
        Node(
            package='pointcloud_to_laserscan',
            executable='laserscan_to_pointcloud_node',
            name='laserscan_to_pointcloud',
            remappings=[('scan_in', '/velodyne_2dscan_high_beams'),
                        ('cloud', '/cloud')],
            parameters=[{'target_frame': 'velodyne', 'transform_tolerance': 0.01}]
        ),
    ])
